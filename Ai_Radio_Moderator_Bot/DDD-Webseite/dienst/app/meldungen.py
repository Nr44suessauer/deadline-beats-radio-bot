...
# Vorspann je Art - so klingt es wie eine Moderation, nicht wie ein Vorleser.
VORSPANN: dict[str, str] = {
    "wetter": "Und nun der Blick aufs Wetter.",
    "nachrichten": "Kurz aus den Nachrichten.",
    "rss": "Neuigkeiten aus dem Netz.",
    "verkehr": "Achtung, eine Verkehrsmeldung.",
    "hinweis": "Ein Hinweis vom Sender.",
    "musik": "",
    # Beim Ueberblick traegt der Titel den Einstieg, sonst kaeme er zweimal.
    "ueberblick": "",
    "sonstiges": "Eine Meldung.",
}
NACHSPANN: dict[str, str] = {
    "wetter": "Das war das Wetter.",
    "nachrichten": "",
    "rss": "",
    "verkehr": "Und weiter geht's mit Musik.",
    "hinweis": "",
    "musik": "",
    "ueberblick": "Das war der Überblick. Und weiter geht es mit Musik.",
    "sonstiges": "",
}
...
def meldungen_erledigt(koerper: Erledigt,
    ...
    if not geaendert:
        return {"ok": False, "geaendert": [], "offen": offen,
                "antwort": "\u26a0\ufe0f Diese Meldung kenne ich nicht mehr.",
                "bearbeiten": True, "tastatur": {"inline_keyboard": []}}
    bild = {"gesagt": "\u2705 Als gesagt vermerkt", "verworfen": "\U0001f5d1\ufe0f Verworfen",
...
def _ansage_meldung_arbeit(anfrage: AnsageMeldung) -> dict[str, Any]:
    ...
    if anfrage.trocken:
        antwort = f"\U0001f3a7 Trockenlauf: {dauer} s - gesprochen wuerde: {text[:200]}"
    else:
        antwort = f"\U0001f399\ufe0f Gesagt ({dauer} s): {titel}"
...
def ansage_text(anfrage: AnsageText,
    ...
    return {"ok": True, "gesprochen": text, "dauer_sekunden": dauer,
            ...
            "antwort": (f"\U0001f3a7 Trockenlauf: {dauer} s" if anfrage.trocken
                        else f"\U0001f399\ufe0f Ansage gesprochen ({dauer} s): {text[:120]}")}
