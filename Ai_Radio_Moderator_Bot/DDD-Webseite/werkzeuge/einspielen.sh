...
#   3. die fuenf neuen einschalten, n8n neu starten (Webhooks anmelden)
#   4. Gesundheit, Testeingang und Ablaufliste pruefen
#
...
echo "--- uebertragen"
for paar in "$K:wf-ddd-k.json" "$W:wf-ddd-w.json" "$A:wf-ddd-a.json" "/tmp/ddd-alt-weg.js:ddd-alt-weg.js"; do
...
ssh -F "$CFG" ai-server "pct exec 103 -- bash -lc '
  set -e
  for f in wf-ddd-k wf-ddd-w wf-ddd-a ddd-alt-weg; do docker cp /tmp/\$f.json n8n:/tmp/ 2>/dev/null || docker cp /tmp/\$f.js n8n:/tmp/ >/dev/null; done
  importiere() {
  ...
  for i in \$(seq 1 40); do
    W1=\$(curl -s -o /dev/null -w \"%{http_code}\" -X POST http://127.0.0.1:5678/webhook/ddd-webseite-test -H \"Content-Type: application/json\" -d \"{}\")
    [ \"\$W1\" != \"404\" ] && break
    sleep 3
  done
