...
AQUA_NAME = os.environ.get("AQUA_TTS_NAME", "aqua")
...
AQUA_ZEITABLAUF = float(os.environ.get("AQUA_TTS_ZEITABLAUF", "600"))
AQUA_ERSATZ = os.environ.get("AQUA_ERSATZ", "de_thorsten")
...
def stimmpfad(name: str) -> Path:
    ...
    raise HTTPException(
        ...
        + ", ".join(sorted({AQUA_NAME, *KURZNAMEN, *(p.stem for p in stimmdateien())})),
    )


def ist_aqua(name: str) -> bool:
    """Ist das der Kurzname der externen Moderationsstimme? (Gross/Klein egal)"""
    return (name or "").strip().lower() == AQUA_NAME.strip().lower()
...
def erzeuge_audio_aqua(text: str, speed: float, format_: str) -> tuple[bytes, str]:
    ...
    zerlegt = urlsplit(AQUA_URL)
    verbindung = http.client.HTTPConnection(
        zerlegt.hostname, zerlegt.port or 80, timeout=AQUA_ZEITABLAUF
    )
    try:
        verbindung.request(
            "POST",
    ...
    if status != 200 or not daten:
        raise HTTPException(
            status_code=502,
            detail=f"Stimmendienst {AQUA_URL} antwortet mit HTTP {status}.",
        )
...
def erzeuge_audio_gewaehlt(text: str, voice: str, speed: float,
    ...
    'aqua' laeuft extern (Stimmendienst); alles andere sind Piper-Stimmen.
    Ist der Stimmendienst nicht erreichbar, wird AQUA_ERSATZ genommen (Standard
    ...
    try:
        daten, typ = erzeuge_audio_aqua(text, speed, format_)
        return daten, typ, gewaehlt
    except Exception as fehler:  # noqa: BLE001 - Ersatzweg statt Ausfall
        ersatz = (AQUA_ERSATZ or "").strip()
        if not ersatz or ist_aqua(ersatz):
            if isinstance(fehler, HTTPException):
                raise
            raise HTTPException(
                status_code=502,
                detail=f"Stimmendienst {AQUA_URL} nicht erreichbar: {fehler}",
            ) from fehler
        print(f"Aqua-Stimme nicht erreichbar ({fehler}) - Ersatzstimme: {ersatz}", flush=True)
...
def health() -> dict[str, Any]:
    return {
        ...
        "aqua": {"name": AQUA_NAME, "url": AQUA_URL, "ersatz": AQUA_ERSATZ},
    }
