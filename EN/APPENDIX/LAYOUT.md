...
It is built from the English workflow copies in `REBUILD/running-workflows/`.
...
| `Message service` | Module news.py in the service | -2320, 1358 |
...
What the bot can do: song requests, direction requests, skip/pause/status, playlists, inbox, research (weather, news, RSS) and announcements on air. Everything comes from Telegram and goes back there. The path of a message: entry -> stage 0/1 analysis -> stage 2 execution -> stage 3 check -> reply. Voice messages run through Whisper at the top, services and inbox hang off to the side. Each node carries its purpose as a note under its name, each frame explains one stage. Frame colours: 1 voice message | 2 entry | 3 services | 4 inbox | 5 stage 0+1 | 6 stage 2 + tools | 7 stage 3 + reply Created by tools/agent-wf-build.py - never change by hand. Change: agent-patch.sh --inhalt ... then agent-import-only.sh. Docs: README.md sections 8 to 12 and tools/README.md.

*(overview box without nodes)*

### Radio - Telegram Agent  ·  `Note docs`

The bot: Telegram entry -> stage 0/1 (understand and plan) -> stage 2 (execute) -> stage 3 (reply). The plan is the source of the layout: tools/agent-wf-build.py (ANORDNUNG, BEREICHE, KURZNOTIZ). Change: agent-patch.sh --aufraeumen -> agent-import-only.sh /tmp/radio-agent-neu.json -> docker restart n8n Check: layout-check.py (0 findings), code-pruefen.py (0 faulty code nodes), DOCS/OPERATIONS.md Description: README.md, DOCS/MANUAL.md, DOCS/MANUAL.md, DOCS/OPERATIONS.md, DOCS/OPERATIONS.md, DOCS/OPERATIONS.md, DOCS/BUILD.md Picture by picture: APPENDIX/n8n-interface.html  (project folder Ai_Radio_Moderator_Bot)
...
Sub-interface of the agent for music: switches, title search, direction, state, playback. It is called through the agent's tool nodes (Tool title search etc.). The plan is the source: tools/agent-wf-build.py (W_ANORDNUNG, W_BEREICHE). Change/check like the agent; description: DOCS/MANUAL.md, DOCS/MANUAL.md, APPENDIX/n8n-interface.html
...
Sub-interface for the station: look up addresses, call an endpoint, give an overview. The addresses come from the station's OpenAPI description (263 endpoints). The plan is the source: tools/agent-wf-build.py (W_ANORDNUNG_AZ, W_AZ_BEREICHE). Change/check like the agent; description: DOCS/MANUAL.md, APPENDIX/n8n-interface.html
...
Sub-interface for announcements and the inbox: research, free announcement, speak/discard a message. Speaks via the radio-tts service (Piper + DJ mount), files messages in the inbox. The plan is the source: tools/agent-wf-build.py (W_MELD_ANORDNUNG, W_MELD_BEREICHE). Change/check like the agent; description: DOCS/MANUAL.md §1.3, DOCS/MANUAL.md
...
First edition of the moderation: create text, speak it, put it on the station. NOT in operation - the live agent (RadioAgentBot) moderates via the radio-tts service. Frames and labels: tools/archive-frame.py (not part of the builder). Description: DOCS/BUILD.md, DOCS/MANUAL.md, APPENDIX/n8n-interface.html (section 16).
...
ONE place for the whole bot: addresses, keys, model, task texts. Only the 'Values' node (code) is edited. Saving is enough, no restart. All four workflows fetch the values at start via their 'Configuration' node. Change: tools/agent-wf-build.py (KONFIG) -> agent-import-only.sh
