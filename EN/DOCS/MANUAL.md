...
(style voice; service `sprechdienst` on the GPU machine, creation: `VOICE.md`),
...
in operation, it has been set softer since 23.09. (high-pass 50 Hz, compression 2:1 from −18 dB, target −12.5 dBFS — see `OPERATIONS.md`). When speaking, the voice is heard in the studio as a streamer **"YOUR-VOICE"**.
...
| Capability | Behavior |
| --- | --- |
| Accept message | `POST /news/neu` (with key) — type, title, text, important, source, URL |
...
| Clean up | alte/erledigte messages removed (`POST /news/aufraeumen`) |
...
| Input | Behavior |
| --- | --- |
| Text message (German or English) | Analyzed in stages (see `MANUAL.md`); the language of the message travels along as the field `sprache` and determines the language of all replies |
...
| Versions | v1–v21 secured, each with description (see `BUILD.md`) |
...
The bot operates in **four stages**. Each stage is a distinct process in the workflow (see `APPENDIX/LAYOUT.md`).
...
`tools/agent-wf-build.py` creates all four workflows; positions, areas,
frames, and labels are tables within. Changes go surgically
over `agent-patch.sh` into the running workflow (never rebuild completely — a full rebuild
...
A FastAPI service with six modules; structure and addresses in `MANUAL.md`.
...
| `catalog.py` | Search index of the archive (48,729 titles), fuzzy and typo-tolerant, Genre/Jahrzehnt |
| `playlist.py` | Build, manage, start playlists (state per chat) |
| `news.py` | Mailbox, moderation, and speech texts, release |
| `search.py` | Research: weather, news feeds, RSS, Wikipedia — and the **topic overview** (press, web, feeds) |
...
the creation of the moderation voice "YOUR-VOICE." The numbers and names come from `README.md`, `MANUAL.md`,
`VOICE.md`, and `VOICE.md`.
...
flowchart TB
  ...
  subgraph P4["Phase 4 · Modell trainieren"]
    TR["rvc-train.sh <your-model> DATASET 400 8<br/>Applio / RVC v2"] --> MOD["<your-model>.pth and <your-model>.index"]
  end

  subgraph P5["Phase 5 · Sprechdienst bauen"]
    DI["speech-service.py as a systemd service<br/>sprechdienst · port 10205"] --> EN["/tts and /health"]
  end
...
`VOICE.md`). If it is not reachable, the service speaks instead with
...
`service/main.py`); `GET /v1/audio/voices` lists `deine-stimme` as an external entry.
...
| `GET /news/offen` | `?anzahl=&art=&nur_neue=` | open messages (Radio-Bot picks them up here) |
| `GET /news/alle`, `GET /news/status` | — | stock and counters |
| `GET /news/text/{kennung}` | — | **preview of the speech text** (what the moderator would say) |
...
`http://192.168.178.26:8888`, setup: `REBUILD/searxng-setup.md`), otherwise
...
`secret.env`). The station displays the display name of the account when speaking,
...
This allows all paths to be tested without real Telegram (see `OPERATIONS.md`).
