...
The speech service **`radio-tts`** (`service/`, port **8881**) turns text into audio and
...
2. Register the short name: either in the `KURZNAMEN` table in `service/main.py` or just
...
bash REBUILD/own-voice/setup-voice-service.sh
...
```bash
bash REBUILD/own-voice/cluster-samples.sh run1 2   # 2 samples per cluster
```
...
| Tool | Task |
| --- | --- |
| `check-voice.py` | window check: clean pieces of the target voice against the reference |
...
/opt/Applio/.venv/bin/python /opt/Applio/stimmen-dienst/collect-voice.py \
...
```bash
bash REBUILD/own-voice/rvc-train.sh <your-model> <your-dataset> 400 8
```
...
The included **`speech-service.py`** is the bridge "text → your voice": base TTS
...
docker cp REBUILD/own-voice/speech-service.py  <container>:/opt/Applio/sprechdienst/speech-service.py
docker cp REBUILD/own-voice/speech-service.service <container>:/etc/systemd/system/
# enter model/index paths, base voice and speed AT THE TOP of the service, then:
systemctl daemon-reload && systemctl enable --now speech-service && systemctl is-active speech-service
...
* **Sharing the GPU:** if a language model (Ollama etc.) runs on the same card, your
  ...
  (`speech-service.py` does), set `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`,
...
All in **`REBUILD/own-voice/`** (adjust the example values inside):

| File | Purpose |
| --- | --- |
| `README.md` | working instructions for the scripts |
| `voice-service.py`, `voice-service.service`, `setup-voice-service.sh` | raw material: video → vocal separation → speaker clusters (port 8890) |
| `cluster-samples.sh` | samples per speaker (choosing the target voice) |
| `check-voice.py`, `collect-voice.py`, `extend-voice.py` | find, collect and expand clean pieces |
| `rvc-train.sh` | training in the container (preprocess → extract → train → index) |
| `speech-service.py`, `speech-service.service` | speech service (port 10205): base TTS → RVC → WAV |
