...
All commands run on your own machine; the tools live in `werkzeuge/`,
the service modules in `dienst/`.
...
| Custom Search Engine (SearXNG) | LXC 108 on `ai-server` (`192.168.178.26`), Port 8888 | `pct exec 108 -- …`; services `uwsgi` + `redis-server`, setup: `NACHBAU/searxng-einrichten.md` |
...
## 4. change the workflow: adjust the builder (agent-wf-bauen.py) and/or patch
bash agent-patchen.sh --inhalt "Knoten A,Knoten B"     # adopt contents
bash agent-patchen.sh --aufraeumen                     # canvas (positions, notes, frames)
...
bash dienst-einspielen.sh        # copies all dienst/*.py + Dockerfile, rebuilds and restarts
```

The call ends with a self-report of the service (`/katalog/status`,
`/meldungen/status`, `/ansage/status`) — this indicates that it is running.

#### 2.3 Change a Setting (Without Rebuilding)

In `/opt/radio-tts/geheim.env` (in LXC 103 on the ai-server), then
...
> `/opt/radio-tts/geheim.env` are `TTS_HOCHPASS_HZ=50`,
...
Rollback: implement the flows from the backup (`agent-einspielen-nur.sh <file>`) and the service modules from `<version>/dienste/`
with `dienst-einspielen.sh` redeploy. The included `n8n-daten.sqlite.gz` is
...
cd ../../werkzeuge
bash playlist/16-ausfuehrungen.sh 4      # latest runs with nodes and results
...
## bot-letzte.js shows: question, stage 0, commands (with "(enqueue)"), overview answer,
...
| n8n | import overwrites `staticData` — the operator list and keys are set via `erlaubte-setzen.py` / `bot-daten-setzen.py` |
...
2. **Rebuild the import files after every change to the builder** (done
   automatically by `agent-patchen.sh`) — otherwise an old version gets deployed.
3. **Never rebuild completely**, patch instead: a rebuild touches the tool nodes
   and changes the tool names seen by the model.
4. **Back up before every change** (`fassung-sichern.sh`).
5. **Check after deploying**: `anordnung-pruefen.py` (0 findings) and a
...
**Rule:** Rebuild import files after any changes to the generator; `agent-patchen.sh` does this automatically now.
...
| Voice too soft in the program | Piper delivers -16.6 LUFS, the music program -9.8 LUFS (the own voice `deine-stimme` runs through the same chain) | Loudness chain (see `HANDBUCH.md`, section 2) |
...
-11.5; in `/opt/radio-tts/geheim.env`, safety next to it). Less compression = the voice sounds less bright; the level is at -12.8 instead of -12.2 dBFS speech RMS. Check: `docker exec radio-tts env | grep TTS_`.
...
**Completed on 2026-09-21:** The own instance runs in **LXC 108** on `http://192.168.178.26:8888` (setup: `NACHBAU/searxng-einrichten.md`), the
...
**Symptom:** `anordnung-pruefen.py` reported in the agent **7 nodes without frames** (`Kurzbefehl?`, `Planen`, `Plan Antwort`, `Plan da?`, `Befehle lesen`, `Befehl da?`, `Kurz?`), a frame **without nodes** and **two overlapping frames** (`Notiz Analyse` / `Notiz Dienste`). In the interface, the blue frame "Level 0 and Level 1" was below the nodes it should explain.
**Cause:** The frames are calculated from the node positions (`werkzeuge/agent-wf-bauen.py`, Tables `BEREICHE`). When a sticky note is moved on the drawing surface with the mouse, this calculation is no longer correct — the nodes then lie beside their frame. (This happens, for example, when working in the interface, if a drag starts on a sticky note: a left drag on an empty area is a selection in n8n, a left drag on a note moves it.)
...
python3 anordnung-pruefen.py /tmp/radio-agent-neu.json    # must report 0 findings
bash agent-einspielen-nur.sh /tmp/radio-agent-neu.json
...
should modify the source and re-run (see `ANHANG/n8n-oberflaeche.html` §18).
...
3. `bash playlist/16-ausfuehrungen.sh 3` — what did the workflow do last?
...
```bash
cd ../../werkzeuge
bash kurz-test.sh            # stage 0: 19 sentences -> expect "19 of 19"
bash antwort-test.sh         # verdict, follow-up, answer, selection buttons -> "20 ok, 0 differing"
python3 anordnung-pruefen.py /tmp/radio-agent-neu.json   # canvas -> "0 findings"
bash playlist/11-dienst-art-test.sh                      # bot input switch -> "37 of 37"
```

| Test | What it ensures |
| --- | --- |
| `kurz-test.sh` / `kurz-test.js` | that level 0 correctly identifies the right sentences (song request, direction, control, status, mailbox query) **and** passes everything else to the AI chain |
| `antwort-test.sh` / `antwort-test.js` | that an empty follow-up response does not clear the output, that a follow-up query is not "followed up," that selection buttons can also come from a compressed list, that a real failure is reported further |
| `anordnung-pruefen.py` | each node in exactly one frame, no overlap, each node with labeling |
| `playlist/11-dienst-art-test.sh` | the switch from Telegram update to decision (buttons, texts, services) with the **real** `EINGABE_JS` |
| `anordnung-doku.sh` | generates `ANORDNUNG.md` anew and checks all four processes during this |

These tests pull their code directly from the generator (`js-holen.py`) — they thus test
...
python3 meldungen/19-meldungen-test.py --live   # additionally a real announcement on air
python3 meldungen/23-lautstaerke-test.py        # 8 samples: volume (calculates + asks the service)
bash playlist/09-dienst-test.sh                 # all playlist paths without Telegram
bash katalog-test.sh                            # Katalogdienst: Kontextsuche, Richtung
bash kontext-test.sh / bash kontext2-test.sh    # interpretation of multiple commands, quantities, references
bash stimme-pruefen.sh                          # finds a speech sample that the recognizer understands cleanly
...
```bash
cd ../../werkzeuge
python3 meldungen/21-bot-meldungen-test.py       # 6 samples: button, tool, release, schedule, cleanup
python3 meldungen/21-bot-meldungen-test.py --live --warten   # with a real announcement and waiting for the schedule
python3 playlist/15-bot-listen-test.py           # playlist paths via the bot
python3 playlist/15-bot-listen-test.py --spielen # additionally play for real (interrupts the broadcast)
...
bash sofort-bot-test.sh / bash sofort-knopf-test.sh  # immediate paths and the button "play immediately anyway"
bash frage.sh "ueberblick ki, raumfahrt"         # Themen-Ueberblick (Ansage im Sender!)
bash frage.sh "topics: photovoltaics from heise golem"   # topics + selected sources
bash frage.sh "give me an overview of ai, but do not read it aloud"   # must NOT speak
bash frage.sh "play Benzin by Rammstein and then Hyper Hyper by Scooter"   # batch command:
expect "plays right now" + "was enqueued"
```

**Important:** `bot-test.sh` does not **accept** any message — it is a fixed suite.
Individual messages go through `frage.sh` / `knopf.sh`; both read the tester key
...
```bash
cd ../../werkzeuge
bash anordnung-doku.sh                       # retrieves the running processes, checks, writes ANORDNUNG.md
python3 anordnung-uebersicht.py <ablauf.json>  # node list as Markdown (screen)
python3 vorschau.py <ablauf.json> bild.png 0.35   # process as image
...
```bash
cd ../../werkzeuge
python3 meldungen/24-live-pegel.py "Test of volume. One, two, three."
...
* **Outage 25 Sep (cause):** `torch.OutOfMemoryError` on CT 111 (card filled to
  59–179 MB free). New in `sprechdienst.py`: CUDA cache is released before/after every
