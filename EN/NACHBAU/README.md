...
| Flows (Bot + 3 tools) | **yes, as creator** — will be rebuilt during setup (`werkzeuge/agent-wf-bauen.py`) |

| Service `radio-tts` (Language, Announcements, Catalog, Lists, Mailbox, Research) | **yes** — `dienst/app/`, `dienst/Dockerfile`, `dienst/docker-compose.yml` |

| Operation and test tools (over 100 scripts) | **yes** — `werkzeuge/` |

| Voices and models | **Guide + download script** (`stimmen-holen.sh`, `stimmen-und-modelle.md`) — the files themselves are 0.2–18 GB |

| Own voice (RVC "YOUR-VOICE") | **yes** — `eigene-stimme/`: all scripts + guide (`README.md`); dataset and model are generated from own media collection |

| Access data | **yes** — `zugangsdaten/` with `UEBERSICHT.md` (working copy: real values 700/600; published: placeholders); how they are created: `zugangsdaten.md` |

| Music archive | **no** — the content itself; the bot runs with any own archive |

| Station configuration | **Guide** — `sender-einrichten.md` |

| Own search engine (SearXNG, LXC 108) | **Guide** — `searxng-einrichten.md` |

| Infrastructure (containers, IPs, proxy) | **Guide** — `umgebung.md` |
...
* A machine with **GPU** for Ollama **and** speech recognition (original: LXC 105 on
  ...
  small model (see `stimmen-und-modelle.md` §3)
* An **AzuraCast** station (original 0.23.4, LXC 106 on the data server). If none is
  running yet, that is the **first** step: `sender-einrichten.md` §0 (installation,
  account, API key)
* **Proxmox VE** as the base if containers are used: `nesting=1` and `keyctl=1` per
  ...
  `umgebung.md` §0 for the short version and the checks
...
machine (`eigene-stimme/README.md` §0). **Time:** with an existing station, Proxmox and GPU,
one long evening is enough (the original took five days of building, `DOKU/BAU.md` §1) —
training your own voice costs one more evening.

Network in original: see `umgebung.md` (fixed IPs, ports 5678, 8881, 8005, 8000,
...
`AZ_URL` and `AZ_KEY`, the address and key of the station (see `dienst/README.md`,
section "Zugangsdatei geheim.env"). Where the steps show example values
(`192.168.x.x`, `YOUR-…`), put in your own.

| # | Step | Result you must see |
| --- | --- | --- |
| 0 | Install the station (`sender-einrichten.md` §0) | `GET /api/status` answers, stream runs |
...
| 6 | Verify everything (`DOKU/BETRIEB.md`) | expectation table 11 of 11 |
...
cp dienst/geheim.env.vorlage /opt/radio-tts/geheim.env   # fill in!
...
> 1. `geheim.env` needs **`AZ_URL`** (address of the station) and **`AZ_KEY`**
>    (API key) — the template `dienst/geheim.env.vorlage` contains **neither**, so add
>    them by hand. Without them the catalogue quietly falls back to an example address.
> 2. The `environment` block of `dienst/docker-compose.yml` sets
>    `TTS_DEFAULT_VOICE`, `EIGENE_STIMME_URL` and `RECHERCHE_SEARX_URL`. Values there
>    **override** `geheim.env`. For a rebuild without an own RVC voice set the default to
...
# Configure the Ollama service (see stimmen-und-modelle.md):
...
#     -> guide: dienst/whisper/README.md  (CUDA or CPU, pulls a ~3 GB model)
...
# systemd unit: see dienst/whisper/README.md
...
> points at your own service (`DOKU/HANDBUCH.md` §4.4, `DOKU/BETRIEB.md` §1).

### Step 3 — Start n8n (two ways)

**Way A — exact copy** (fast, requires old access values):
`ablaeufe-laufend/*.json` import (**seven** files: the hub `Konfiguration`, the agent,
...
tools, then the agent; the **five** workflows listed in `vorlage.md` §4 are switched on.
Guide in `ablaeufe-laufend/README.md`.
...
```bash
# export the credentials into the environment (see zugangsdaten.md)
...
cd werkzeuge
python3 agent-wf-bauen.py            # writes /tmp/radio-konfiguration.json,
                                     # /tmp/radio-werkzeuge.json + /tmp/radio-agent.json
python3 anordnung-pruefen.py /tmp/radio-agent.json     # must report "0 Befunde"
python3 import-agent-vorbereiten.py  # writes /tmp/radio-agent-import.json
...
> changes were always **surgically patched** (`werkzeuge/agent-patchen.sh`). To exactly replicate the running bot,
> take the exports from `ablaeufe-laufend/` (step 3, way A — they contain the
...
```bash
cd werkzeuge
# adapt target host/project in agent-einspielen-nur.sh + import-agent-vorbereiten.py
...
reply when the key is right — the call and the expected output are in `DOKU/BETRIEB.md` §3.

Afterwards:
* Check the test entry webhook — see `DOKU/BETRIEB.md` §3 for the call and the expected output
* Enable the operator: `python3 erlaubte-setzen.py <chatId>`
* Set bot data: `python3 bot-daten-setzen.py`
* Telegram menu: `bash telegram-menue.sh`

### Step 5 — Set Up the Station

Follow `sender-einrichten.md`: Station, Mount, Streamer access, Playlist,
...
Order (details in `../DOKU/BETRIEB.md`):

```bash
bash kurz-test.sh          # 19 of 19
bash antwort-test.sh       # 20 ok
bash playlist/11-dienst-art-test.sh          # 37 of 37
python3 meldungen/19-meldungen-test.py       # 45 ok   (needs the service)
python3 meldungen/21-bot-meldungen-test.py   # 6 ok    (braucht n8n + Testeingang)
bash bot-test.sh "what is playing"                 # Antwort in ~1–2 s
python3 meldungen/24-live-pegel.py "Test of the volume."   # speaks on air
```

### Step 8 — Personal Voice (optional)

Those who want to speak announcements with a transformed voice (RVC) build it according to
`eigene-stimme/README.md`: separate raw material (voice service, port 8890), get confirmation from the operator,
collect pure pieces (`stimme_erweitern.py`), train the model (`rvc-trainieren.sh <your-model> … 400 8`), start the speech service `sprechdienst` (port 10205)
— short test:
...
(`TTS_DEFAULT_VOICE`, step 1); without a voice service, a Piper template suffices (`TTS_DEFAULT_VOICE=de_thorsten`). Details: `eigene-stimme/README.md`, section 8.
A **general** guide (each Serie/Stimme, all values and commands): `../DOKU/STIMME.md`.
...
| `dienst/` *(in the project folder, `../dienst/`)* | the modules of the speech service (`main.py`, `katalog.py`, `playlist.py`, `meldungen.py`, `suche.py`), `Dockerfile`, `docker-compose.yml`, `geheim.env.vorlage`, `whisper/` (fallback speech recognition), `whisper-amd/` (speech recognition on the MI50) |

| `werkzeuge/` *(in the project folder, `../werkzeuge/`)* | all operation, build, and test scripts (over 100 files) |

| `zugangsdaten.md` | which access credentials are needed and how they are created |

| `umgebung.md` | containers, IPs, ports, services, reverse proxy |

| `sender-einrichten.md` | set up the station (AzuraCast) |

| `stimmen-und-modelle.md` | voices and models: sources, sizes, configuration |

| `stimmen-holen.sh` | loads the four Piper voices into a target directory |

| `eigene-stimme/` | **rebuild your own voice**: Extraction service (8890), collector (`deine-stimme_sammeln3/4/5.py`), training script, speech service (10205), test tools — guide in the folder |

| `ablaeufe-laufend/` | **the exports of the running workflows** (exact restoration; include access values, 700/600) |

| `zugangsdaten/` | **the actual access values** (700/600; placeholders in the published copy) with `UEBERSICHT.md`: Telegram token, chat IDs, mailbox and test keys, AzuraCast keys, DJ password, `geheim.env`, n8n access, SSH keys |

**Included but handle with care:** `zugangsdaten/` and
`ablaeufe-laufend/` — both contain the actual access values (permissions 700/600).
...
1. `anordnung-pruefen.py` reports **0 findings** (identical surface),
...
6. an announcement on the radio is audible (volume: around −11.8 LUFS on broadcast, see
   `../DOKU/HANDBUCH.md`, section 2).

---

## 5. What Can Be Different

* **Voice**: The default is the **own speaker voice `deine-stimme`** (external, `eigene-stimme/`);
  without voice service, set `TTS_DEFAULT_VOICE` to a Piper voice (short names in
  `dienst/app/main.py`). If the voice service fails, `radio-tts` continues with
...
* **Station**: every AzuraCast installation with Liquidsoap/AutoDJ works; it must
  meet the requirements listed under `sender-einrichten.md` (DJ Harbor,
  API key).
...
> **The access data is now in the folder** — in `zugangsdaten/`
> (directory 700, files 600, with `UEBERSICHT.md`). Anyone who shares the folder
> shares the bot: Telegram token, station key, DJ password,
  inbox-/Testschlüssel and the SSH keys. For a release without secrets,
  simply omit `zugangsdaten/`.
...
3. **The station database** (playlists, users, history). → AzuraCast backup
   (`azuracast_cli backup`) or rebuild according to `sender-einrichten.md`.
...
6. **Raw material, dataset, and model of the own speaker voice** — come from
   ...
   the folder; the rebuild from the own stock is described in `eigene-stimme/README.md`.
...
| Building workflows (`python3 werkzeuge/agent-wf-bauen.py`) | Bot with 83 nodes + 3 tools, all connections verified |

| Canvas (`python3 werkzeuge/anordnung-pruefen.py /tmp/radio-*.json`) | **0 findings** |

| Python syntax of all modules (`compileall`) | in order |

| `docker compose config` with the template | valid, all variables resolved |

| `docker build` from `dienst/` | Image built (base `python:3.12-slim` + piper-tts, fastapi, lameenc) |
...
| Loading voices (`stimmen-holen.sh`) and **checksums** comparison | 4 voices, md5 **identical** with the running system |
