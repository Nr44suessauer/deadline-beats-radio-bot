...
are in the Proxmox documentation ("PCI(e) Passthrough") and in `DOKU/BETRIEB.md` §7 (too
...
| **Search Engine** | LXC 108 on `ai-server` (`192.168.178.26`, Port 8888) | SearXNG for topic overview (Setup: `searxng-einrichten.md`) | 2 cores, 1 GB RAM, 10 GB disk |
...
├── Dockerfile
├── docker-compose.yml
├── geheim.env             (600) DJ port + volume control values
