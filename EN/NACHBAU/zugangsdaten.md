...
| **Liquidsoap/streamer passwords** | live announcements to the station (and manual sending) | AzuraCast: **two** streamer accounts to create — one for the operator (e.g., `operator`), one for the bot (e.g., `deine-stimme`, display name "YOUR-VOICE") | bot account as `LIVE_USER`/`LIVE_PASSWORD` in `/opt/radio-tts/geheim.env` |

| **Mailbox Key** | accept messages from outside | self-generated (32 characters) | `radio/meldung-schluessel.txt` + `geheim.env`/`X-Meldung-Schluessel` |
...
## 2. Service Environment File (`/opt/radio-tts/geheim.env`)

Template: `dienst/geheim.env.vorlage` — copy, fill out, set permissions to 600.
...
3. Determine your own chat ID and enter it in the operator list:
   ...
   ```
4. Command menu (optional): `bash telegram-menue.sh`.
...
move to `geheim.env` (`LIVE_USER`, `LIVE_PASSWORD`).
...
python3 bot-daten-setzen.py             # test key and base state
python3 statische-daten-patchen.py      # keep existing data, only additions
```
...
| `/opt/radio-tts/geheim.env` (LXC 103) | DJ harbor and volume control values |
