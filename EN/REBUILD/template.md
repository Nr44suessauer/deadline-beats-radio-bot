...
1. **A URL component must be built as a COMPLETE expression.** n8n does not resolve
   ...
   The creator handles this via the class `Ausdruck` (see `agent-wf-build.py`):
...
```bash
cd ../..
VORLAGE=1 VORLAGE_ZIEL=/tmp/radio-vorlage python3 tools/agent-wf-build.py
```
...
```bash
python3 tools/config-check.py     # 5 checks (names, references,
...
- **Backups are now harmless**: the outputs of `02-werkzeuge.json` and
  ...
  shared.
- The check `config-check.py` is new and runs on every change.