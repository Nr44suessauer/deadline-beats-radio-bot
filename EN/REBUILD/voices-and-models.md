...
are too large for a documentation folder — the Piper voices are fetched from `fetch-voices.sh`,
the models pull their tools themselves. The **own speaker voice** (RVC) is trained from
personal material — Creation: `../DOCS/VOICE.md`, Replication: `own-voice/`.
...
`/opt/radio-tts/secret.env`, then `docker compose up -d`.
...
| Service | `service/whisper/whisper_server.py`, Port **18790**, systemd `whisper-stt` |
...
cp service/whisper/whisper_server.py /root/whisper-stt/
python3 -m pip install faster-whisper
# systemd unit see service/whisper/README.md, then:
