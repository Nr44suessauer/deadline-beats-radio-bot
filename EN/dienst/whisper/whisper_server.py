...
def _zerlege_multipart(body: bytes, boundary: str):
 ...
 Return: (audio_bytes|None, {feldname: wert})
    """
    audio = None
    felder = {}
    for teil in body.split(boundary.encode()):
        ...
        # Remove trailing line breaks and boundary markers
...
class WhisperHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # Language: Standard German - the broadcaster is operated in German. "auto"
        # releases the recognition again.
        voice = (felder.get("language") or "de").strip().lower()
        if voice in ("", "auto", "none", "null"):
            voice = None
        hint = (felder.get("prompt") or "").strip() or None

        # faster-whisper reads the file itself (recognition via ffmpeg/PyAV).
        try:
        ...
        try:
            print(f"[whisper] Transcribing {len(audio_data)} bytes (lang={voice}) ...",
                  flush=True)
