...
├── catalog.py          # Katalogdienst: /suche, /genre, /genre/liste, /katalog/*
├── playlist.py         # Listenaufgaben: /playlist/befehl, /playlist/knopf, /playlist/vorschlag
├── news.py        # inbox and announcements: /news/*, /announce/*
├── search.py            # Recherche: /recherche (Wetter, Nachrichten, RSS, Wikipedia)
...
**Attention when building:** the Dockerfile must **copy every** module (`COPY app/news.py /app/news.py` …) – otherwise the service
`No module named 'meldungen'` will report missing addresses.
Deployment and verification: `bash ../tools/service-import.sh`,
`bash ../tools/news/19-news-test.py`.

## Server Setup

```bash
CFG=~/.ssh/config
# copy the sources over (example catalog.py)
cat service/catalog.py | ssh -F $CFG ai-server "pct exec 103 -- bash -c 'cat > /opt/radio-tts/app/catalog.py'"
# build and start
...
Check: `bash ../tools/catalog-test.sh http://192.168.178.53:8881`

## Access File `secret.env` (600, not in Git)
...
For the announcement, the service needs the announcement access (`LIVE_*` in `secret.env`;
own bot account `deine-stimme`, in the station appears as "YOUR-VOICE" when speaking),
configured with `../tools/news/18-live-access-set.sh`.
Detailed: `../tools/news/README.md` and `../README.md` §12.
