...
# import-agent-prepare.py only reads /tmp/radio-*.json.
python3 agent-wf-build.py
python3 import-agent-prepare.py
...
docker cp /tmp/agent-postprocess.py n8n:/tmp/ >/dev/null
...
python3 /tmp/agent-postprocess.py "$KEY" $IDS
...
for file in /tmp/radio-werkzeuge-import.json /tmp/radio-agent-import.json /tmp/ollama-cred.json \
             agent-postprocess.py /tmp/agent-remote.sh; do
