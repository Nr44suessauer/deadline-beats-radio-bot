...
#
# Call:  bash agent-patch.sh [“--new A,B --content C --rename ‘X=Y’”]
...
python3 import-agent-prepare.py >/dev/null

echo "--- patching"
UMB=$(cat /tmp/rename.txt 2>/dev/null || true)
python3 agent-patch.py "$RUNNING" /tmp/radio-agent.json "$ZIEL" --rename "$UMB" "$@"
