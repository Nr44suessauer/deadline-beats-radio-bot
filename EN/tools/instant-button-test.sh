#!/usr/bin/env bash
# Tests the “Still play immediately” button (s:).
WEBHOOK=http://127.0.0.1:5678/webhook/YOUR-WEBHOOK-PATH
echo "════ /wish benzin (places the plan)"
curl -s -m 60 -o /dev/null -w " HTTP %{http_code}\n" -X POST "$WEBHOOK" -H 'Content-Type: application/json' \
  -d '{"message":{"message_id":1,"chat":{"id":1,"type":"private"},"from":{"id":1,"first_name":"Test"},"text":"/wish benzin"}}'
echo "════ Button s: (Still play immediately)"
curl -s -m 60 -o /dev/null -w " HTTP %{http_code}\n" -X POST "$WEBHOOK" -H 'Content-Type: application/json' \
