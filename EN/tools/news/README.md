...
The logic itself lives in **`../../service/news.py`** and runs in the
...
`POST /announce/item`. Test run: `bash ../short-test.sh` (19 cases).
...
sequenceDiagram
  ...
  participant D as service radio-tts (news.py)
...
```bash
python3 19-news-test.py     # contains the research samples (dry)
```
...
`22-searchbot-example.py` in this folder is a runnable example: it deposits a
weather and a feed message and shows the preview of the spoken text.

```bash
python3 22-searchbot-example.py --dry      # only file it, speak nothing
```
...
```bash
cd ../../tools/meldungen
python3 19-news-test.py            # service: text preparation and all paths
python3 19-news-test.py --live     # additionally a real announcement on air
python3 21-bot-news-test.py        # bot: buttons, tool, release
python3 21-bot-news-test.py --live --warten   # with announcement and waiting for the schedule
python3 23-volume-test.py          # volume of the voice (calculates + asks the service)
python3 24-live-level.py                # test announcement + recording: measure voice against music
bash 18-live-access-set.sh           # enter the announcement account (YOUR-VOICE) into the service configuration
```

Deploying the service module (from `tools/`):

```bash
bash service-import.sh      # copies main.py, catalog.py, playlist.py, news.py + Dockerfile
```
