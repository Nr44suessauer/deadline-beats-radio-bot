...
The logic itself lives in **`../../service/playlist.py`** and runs in the
...
python3 10-split-test.py        # 1) check parsing (sentences -> job)
bash 11-service-type-test.sh        # 2) check the switch (Telegram update -> decision)
...
```bash
cd ../../werkzeuge
bash version-save.sh radio-vX-<date>-<shortname> /tmp/description.md
```

Back to the saved state (bot workflow only):

```bash
bash ../tempo/04-import.sh \
...
| File | What it does |
| --- | --- |
| `05-playlist-paths-test.sh` | earlier test run of the station addresses (create/extend/clear) |
| `06-clear-rename-test.sh`, `07-rename-test.sh` | single checks from the build phase |
| `08-service-import.sh` | copies `service/playlist.py` into the service, wires the router into `main.py`, rebuilds and restarts the service |
| `09-service-test.sh` | 13 steps: show lists, menu, tick, create, cancel, view, rename, clear, delete, list selection, unknown job |
| `10-split-test.py` | job parsing without service and without network (mocks for FastAPI/Pydantic) |
...
`agent-import.sh` would rebuild the **whole** workflow. The builder sets the
...
does not belong to this extension. `13-playlists-patch.py` therefore patches
surgically:
...
Full version: `../../README.md` §11 and `../../DOCS/MANUAL.md` §5.
