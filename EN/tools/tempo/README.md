...
go to the AI. Checked with `02-short-test.js` (19 cases, all as expected).
...
The first version of `03-tempo-patch.py` masked the credentials **for the
...
| File | Purpose |
|---|---|
| `01-version-save.sh` (removed) | create a version backup of the workflows (+ n8n database) |
| `02-short-test.js` | check the pre-stage (`Short?`) with 19 example sentences – without n8n |
| `03-tempo-patch.py` | adopt the changed nodes from the rebuild into the running version |
| `04-import.sh` | deploy the patched workflow into n8n and restart n8n |
