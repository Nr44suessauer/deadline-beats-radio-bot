...
#
# Takes a finished import file (from agent-patchen.sh) for the agent and the
...
[ -f "$TOOLS" ] || { echo "File missing: $TOOLS (run agent-patchen.sh first)"; exit 1; }
[ -f "$KONFIG" ] || { echo "File missing: $KONFIG (first run agent-patchen.sh)"; exit 1; }
