#!/usr/bin/env python3
"""Builds ANHANG/n8n-oberflaeche.html: the bot in the n8n interface.
...
The images are created with `modulbilder-plan.py` (plan), the capture in the
running interface (tiles) and `images-stitch.py` (assembly).

Usage: python3 werkzeuge/n8n-doku-build.py
"""
...
TEXTE = {
    ...
    "Note Old Tools": "leftovers of the first version — not connected, only for "
                              "Erinnerung.",
    # --- Configuration - all values --------------------------------------
    "Note hub": "The hub of the bot: the node <code>Values</code> holds all "
                      "addresses, keys and task texts in ONE place. Changed "
                      "is only this code node — saving is enough, no restart.",
    # --- Voices from films ----------------------------------------------
    "Note 214": "The start: the form <code>…/form/voices-out-filmen</code> or "
                   "the webhook accept series and job name.",
    "Note 566": "The search: <code>/finden</code> finds series or film in the "
                   "libraries, <code>/extrahieren</code> starts the job at the "
                   "voice service (port 8890).",
    "Note 132": "Wait and report: the loop asks <code>/job</code>, aborts after "
                   "60 minutes (the job continues in the container) and sends "
                   "result or error via Telegram.",
}

ABLAEUFE = [
    ("RadioAgentBot", "Radio - Telegram-Agent", "in operation · 83 nodes in 10 modules"),
    ("RadioWerkzeug", "tool - Radio", "in operation · 17 nodes in 7 modules"),
    ("AzuraWerkzeug", "tool - AzuraCast", "in operation · 17 nodes in 5 modules"),
    ("MeldungenWerkzeug", "Tool - Messages", "in operation · 14 nodes in 6 modules"),
    ("Configuration", "Configuration - all Values", "in operation · 2 nodes in 2 modules"),
    ("StimmenBot", "Stimmen out Filmen", "in operation · 20 nodes in 3 modules"),
...
def bild_html(file: str, beschriftung: str, klasse: str = "modul") -> str:
    ...
    # mask the alt attribute (quotes in captions would otherwise break tags)
...
<ul>
  ...
  <li><strong>Seven radio workflows:</strong> agent, three tools, hub
      ("Configuration – all Values"), "Stimmen out Filmen" and the archive (chapter below)</li>
  <li><strong>Rule:</strong> in the interface only <em>view</em> — the plan is changed
      in <code>NACHBAU/bau/agent-wf-build.py</code></li>
</ul>
</div>""")
...
{bild_html("modul-radio-telegram-agent-10.png", "The documentation note of the agent")}
{bild_html("modul-radio-telegram-agent-09.png", "The legend (overview note) explains the colours of the areas")}
...
{tabelle([
    ("Change the workflow", "<code>agent-patchen.sh --cleanup</code> → <code>agent-deploy-only.sh /tmp/radio-agent-new.json</code> → <code>docker restart n8n</code>"),
    ("Check the layout", "<code>python3 werkzeuge/anordnung-check.py NACHBAU/workflows-running/*.json</code> (goal: 0 findings)"),
    ("Capture images again", "<code>modulbilder-plan.py</code> → capture tiles → <code>images-stitch.py</code> → <code>n8n-doku-build.py</code>"),
    ("Back up a version", "<code>bash werkzeuge/version-sichern.sh &lt;name&gt; [description.md]</code>"),
])}""")
...
<header>
  ...
  <p>{count} modules in {len(plan)} workflows · one image per module, clickable to zoom · {BAUZEIT}</p>
</header>
<main>
{seiten}
</main>
<footer>
  <p>Generated by <code>werkzeuge/n8n-doku-build.py</code> from the module images in
  <code>ANHANG/images/module/</code> (plan: <code>modulbilder-plan.py</code>, capture in
  the running interface, assembly: <code>images-stitch.py</code>).</p>
  <p>Nothing was changed on the bot; node lists: <code>ANHANG/ANORDNUNG.md</code>.</p>
</footer>
...
print(f"{AUSGABE} written: {groesse:.2f} MB, {len(html)} characters, "
      f"{count} modules + {len(plan)} overviews")
