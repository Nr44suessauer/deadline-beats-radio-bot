...
for F in agent-wf-build.py agent-deploy.sh agent-nachbereiten.py \
         import-agent-vorbereiten.py statische-data-patchen.py allowed-setzen.py \
         code-check.py archiv-anlegen.py schlagwort-anlegen.py \
         bot-test.sh antworten.js letzter.js eine.js feld.js auswertung.py \
         durchlauf.sh ausfuehrungen.sh version-sichern.sh \
         agent-patchen.py agent-patchen.sh agent-deploy-only.sh service-deploy.sh; do
...
cp "$RADIO/HANDBUCH.md" "$ZIEL/ARCHITEKTUR-the-Doku.md" 2>/dev/null || true
...
bash werkzeuge/playlist/08-service-deploy.sh
\`\`\`
