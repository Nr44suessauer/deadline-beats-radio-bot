...
Call:
    python3 22-suchbot-beispiel.py                 # file a weather and feed message
    python3 22-suchbot-beispiel.py --weather        # weather only
    python3 22-suchbot-beispiel.py --important       # as an important message
    python3 22-suchbot-beispiel.py --cleanup    # eigene Proben again remove
...
def news(type: str, title: str, text: str, important: bool = False,
    ...
    body = json.dumps({"type": type, "title": title, "text": text, "important": important,
                          "source": source, "url": url, "from": "22-suchbot-beispiel.py"})
...
def cleanup() -> None:
    ...
    meine = [m["id"] for m in all if m.get("from") == "22-suchbot-beispiel.py"]
    if not meine:
