...
Wichtige Schnittstellenwege des Senders (am 2026-09-20 geprueft):
    GET    /api/station/1/playlists                     Listen (id, name, num_songs)
    GET    /api/station/1/playlist/{id}/export/m3u      Titel als Pfade, in Reihenfolge
    PUT    /api/station/1/files/batch                   {do:playlist, files:[...],
                                                         playlists:["new"],
                                                         new_playlist_name:"..."} legt an
                                                         UND fuellt in einem Aufruf
    POST   /api/station/1/playlist/{id}/import          m3u hochladen -> ergaenzt
    DELETE /api/station/1/playlist/{id}/empty           leert die Liste
    DELETE /api/station/1/playlist/{id}                 loescht die Liste
    PUT    /api/station/1/playlist/{id}                 aendern (ohne "links"/"podcasts"!)
...
def listen() -> list[dict[str, Any]]:
    code, daten = _api("GET", "/api/station/1/playlists")
...
def liste_inhalt(pid: int) -> tuple[list[str], list[str]]:
    ...
    pfade: list[str] = []
    titel: list[str] = []
    code, text = _api("GET", f"/api/station/1/playlist/{pid}/export?format=m3u", roh=True)
    if code == 200:
    ...
    if not pfade:  # Ersatzweg: reine Pfadliste
        code2, text2 = _api("GET", f"/api/station/1/playlist/{pid}/export/m3u", roh=True)
        if code2 == 200:
...
def liste_anlegen(name: str, pfade: list[str]) -> dict[str, Any]:
    code, daten = _api("PUT", "/api/station/1/files/batch", {
...
def liste_ergaenzen(pid: int, pfade: list[str]) -> tuple[bool, str]:
    ...
    inhalt = ("\n".join(pfade) + "\n").encode("utf-8")
    code, daten = _api("POST", f"/api/station/1/playlist/{pid}/import",
...
def liste_umbenennen(pid: int, neu: str) -> tuple[bool, str]:
    code, daten = _api("GET", f"/api/station/1/playlist/{pid}")
    ...
    koerper["name"] = neu
    code, antwort = _api("PUT", f"/api/station/1/playlist/{pid}", koerper)
    if code != 200:
...
def liste_abspielen(pid: int, pfade: list[str] | None = None) -> tuple[bool, str]:
    ...
    code, _ = _api("PUT", "/api/station/1/files/batch", {"do": "immediate", "files": [titel[0]]})
    if code != 200:
        return False, f"Start fehlgeschlagen (HTTP {code})"
    if len(titel) > 1:
        _api("PUT", "/api/station/1/files/batch", {"do": "queue", "files": titel[1:]})
...
def kandidaten(suchtext: str, anzahl: int = KANDIDATEN_MAX) -> list[dict[str, Any]]:
    ...
    if len(treffer) < anzahl:
        ...
        code, daten = _api("GET", "/api/station/1/files?rowCount=30&searchPhrase="
...
def menue_lieder(z: dict[str, Any]) -> str:
    zeilen = [f"{z['name']}" if z.get("name") else "Neue Wiedergabeliste",
              f"Suchbegriff: {z.get('kriterium', '')}", ""]
    for i, k in enumerate(z["kandidaten"], start=1):
        zeilen.append(("[x] " if k["gewaehlt"] else "[ ] ") + f"{i}. {k['titel']}")
...
def menue_tastatur_lieder(z: dict[str, Any]) -> dict[str, Any]:
    ...
    zeilen.append([{"text": "Fertig", "callback_data": "pf"},
...
def menue_tastatur_listen(listen_: list[dict[str, Any]]) -> dict[str, Any]:
    zeilen = [[{"text": f"{p['name']} ({p.get('num_songs', 0)} Titel)"[:60],
...
def playlist_befehl(befehl: Befehl) -> dict[str, Any]:
    ...
    if art == "listen":
        ...
        zeilen = ["Wiedergabelisten:"]
        for p in alle:
            zeichen = "[an]" if p.get("is_enabled") else "[aus]"
...
def _weitere_aktion(chat: str, art: str, liste: dict[str, Any], auftrag: dict[str, Any]) -> dict[str, Any]:
    ...
    if art == "abspielen":
        ...
        return {"antwort": meldung, "bereich": "playlist"}
    if art == "ansehen":
    ...
    if art == "umbenennen":
        ...
        return {"antwort": (f"\u201e{name}\u201c heisst jetzt \u201e{neu}\u201c."
                            if ok else meldung), "bereich": "playlist"}
    if art == "ergaenzen":
...
def playlist_knopf(knopf: Knopf) -> dict[str, Any]:
    ...
    if daten == "px":
        ...
        antwort = ("Alles klar, \u201e" + str(z["name"]) + "\u201c bleibt gespeichert."
    ...
    if z.get("art") == "bestaetigung" and daten in ("j", "n"):
        ...
        if z["was"] == "leeren":
            ...
            return {"antwort": f"\u201e{z['name']}\u201c ist geleert." if ok
                    else "Leeren hat nicht geklappt.",
                    "bearbeiten": True, "tastatur": {"inline_keyboard": []}, "bereich": "playlist"}
        ok = liste_loeschen(int(z["pid"]))
        return {"antwort": f"\u201e{z['name']}\u201c ist geloescht." if ok
                else "Loeschen hat nicht geklappt.",
                "bearbeiten": True, "tastatur": {"inline_keyboard": []}, "bereich": "playlist"}
