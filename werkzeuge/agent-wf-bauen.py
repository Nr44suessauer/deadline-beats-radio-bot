...
class Ausdruck(str):
    ...
    zusammengesetzte Adressen so aussehen:
        ...
        AZ + "/api/station/1"  ->  ={{ $('Konfiguration').first().json.konfig.sender.adresse
                                       + "/api/station/1" }}
    """
...
# ---------------------------------------------------------------- Charakter
# Der Charakter der STIMME steht als Klartext in einer Datei neben dem Projekt
...
# Er faerbt NUR die GESPROCHENEN Ansagen: der Knoten "Planen" haengt ihn zur
# LAUFZEIT an seinen Systemtext und formuliert damit freie Ansagen (art=ansage,
# z. B. "sag eine Begruessung an"). Die Telegram-Antworten bleiben sachlich -
# die Ausfuehrung bekommt den Charakter absichtlich NICHT.
...
def modell_koerper(aufgabe, nutzer_ausdruck, temperatur, tokens, zusatz=""):
    ...
    `zusatz` ist ein Ausdrucksstueck, das HINTEN an den Systemtext haengt - beim
    Planen der Charakter der Stimme fuer gesprochene Ansagen (CHARAKTER_PLAN).
    ...
    return ("={{ JSON.stringify({ model: " + K + ".sprachmodell.modell"
            + ", messages: [{ role: 'system', content: " + K + ".aufgaben." + aufgabe + zusatz
...
W_MELDUNGEN_NAME = "Werkzeug - Meldungen"
# Der Dienst, der das Postfach fuehrt (LXC 103, radio-tts)
# Der Dienst, der das Postfach fuehrt (LXC 103, radio-tts). Die Adresse steht in der
...
if (roh.error) {
  return [{ json: Object.assign({}, felder, { text: '',
    antwort: 'Die Sprachnachricht konnte nicht verarbeitet werden.' }) }];
}
...
return [{ json: { chatId: j.chatId || $('Eingabe').item.json.chatId,
  antwort: (EN ? 'Understood: \u00bb' : 'Verstanden: \u00bb')
    + (j.gehoert || '') + '\u00ab' } }];
"""
...
// Dienst radio-tts (playlist.py fuer Wiedergabelisten, meldungen.py fuer das
...
const zeilen = [
  '<b>' + (m.wichtig ? 'Wichtig: ' : '') + entkommen(kopf) + '</b>',
];
...
const tastatur = { inline_keyboard: [[
  { text: 'Vorlesen', callback_data: 'm' + m.id },
  { text: 'Verwerfen', callback_data: 'x' + m.id },
]] };
...
if (!text) text = 'Das Listen-Modul hat nicht geantwortet.';
...
5. art=ansage: der Betreiber will eine Ansage im Radio hoeren. Zwei Faelle:
   a) Er gibt den Wortlaut vor ("sag durch: ...", "lies folgenden Text vor: ...", englisch
      "announce: ...", "say on air: ...", "read this out: ..."): "text" uebernimmst du WOERTLICH
      und in seiner Sprache - nichts umformulieren, nichts ergaenzen.
   b) Er laesst den Wortlaut dir ("sag eine Begruessung an", "gruesse die Hoerer", "sag ein
      paar nette Worte an", "verabschiede die Hoerer"): FORMULIERE den gesprochenen Text
      selbst - ein, zwei kurze Saetze (Richtwert 200 Zeichen), in der Sprache des Betreibers
      und im Stil der Rolle unten ("DEINE STIMME"), falls eine mitgegeben ist. Beispiel:
      "sag eine Begruessung an" -> {"art": "ansage", "text": "Hallo ihr Lieben, schoen, dass
      ihr da seid!"}
   NICHT fuer Musikwuensche, Fragen oder Nachrichten aus dem Netz (dafuer recherche).
6. art=recherche: etwas nachschlagen und im Radio ansagen - Wetter, Nachrichten, ein Feed, ein
...
7. art=verwalten: alles am Sender selbst (Wiedergabelisten, Anlagen, Nutzer, Rollen, Sicherungen,
   Einstellungen, Berichte, Medien, Streamer). Schreibe den Auftrag vollstaendig in eigenen Worten
   in "auftrag".
8. bestaetigt=true NUR, wenn der Betreiber gerade ausdruecklich zugestimmt hat ("ja", "mach das",
...
9. Verhoerte Namen dem naechstliegenden Interpreten zuordnen. Nichts erfinden.
10. Mehrere Schritte einer zusammenhaengenden Aufgabe ("lege eine Wiedergabeliste an und fuelle sie
...
11. Steht in der Nachricht keine Aufgabe, gib {"befehle": []} zurueck.
...
for (let i = 0; i < befehle.length; i += 1) {
  ...
  // Ausdruecklicher Stimmenwunsch ("mit eigener Stimme", "in eigener Stimme", ...) - fest
...
3. Antworte in EINEM kurzen Satz mit dem Ergebnis - in der Sprache des Betreibers
   ...
   Der Telegram-Kanal ist ein sachlicher Dienstkanal: keine Emojis, keine Ausrufezeichen,
   keine Anreden, keine Floskeln, keine Bewertungen - nur das Ergebnis.
...
# Charakter der STIMME fuer den Planer. Der Text haengt zur LAUFZEIT an der Zentrale
# (Feld "charakter") - so wirkt eine geaenderte Charakterdatei nach dem Einspielen
# der Zentrale, ohne Neubau. Die Rolle faerbt NUR das Feld "text" bei art=ansage:
# diesen Text formuliert der Planer selbst, wenn der Betreiber es so will, und er
# wird im Sender gesprochen. Telegram-Antworten bleiben sachlich (ohne Charakter).
CHARAKTER_PLAN = (" + (" + K + ".charakter ? '\\n\\nDEINE STIMME (so klingt deine Rolle, "
                  "wenn du im Sender sprichst)\\n' + " + K + ".charakter"
                  + " + '\\nDiese Rolle gilt NUR fuer das Feld \"text\" bei art=ansage - dieser "
                    "Text wird gesprochen. Der Plan selbst bleibt sachlich und exakt im Format "
                    "oben.' : '')")

# Systemtext der Ausfuehrung: Aufgabentext + "/no_think". Den Charakter der Stimme
# bekommt die Ausfuehrung ABSICHTLICH nicht: sie schreibt die Telegram-Antwort
# (sachlich) und spricht nur den Text, den der Plan vorgibt.
SYSTEM_AUSFUEHREN = Ausdruck(K + ".aufgaben.ausfuehren" + " + '\\n/no_think'")
...
bot = [
    ...
    # und Sprachmodell, sondern direkt in den Dienst radio-tts (playlist.py).
    ...
    n("Meldung Dienst", "n8n-nodes-base.httpRequest", 4.2, [-2320, -960], {
       ...
       notes="Meldungs-Modul im Dienst radio-tts: legt das Postfach des Suchbots "
    ...
    n("Listen Dienst", "n8n-nodes-base.httpRequest", 4.2, [-2320, -1220], {
       ...
       notes="Listen-Modul im Dienst radio-tts: sucht, baut, verwaltet und spielt "
    ...
    n("Dienst Ersatz senden", "n8n-nodes-base.httpRequest", 4.2, [-1440, -1220], {
        ...
        "jsonBody": ("={{ JSON.stringify(Object.assign({ chat_id: $('Dienst Antwort').first().json.chatId,"
                     " text: 'DBeats: ' + $('Dienst Antwort').first().json.antwort, parse_mode: 'HTML',"
    ...
    n("Dienst Senden", "n8n-nodes-base.httpRequest", 4.2, [-1880, -1220], {
        ...
        "jsonBody": ("={{ JSON.stringify($json.bearbeiten ? { chat_id: $json.chatId,"
                     " message_id: $json.nachrichtId, text: 'DBeats: ' + $json.antwort, parse_mode: 'HTML',"
                     " disable_web_page_preview: true, reply_markup: $json.tastatur ||"
                     " { inline_keyboard: [] } } : Object.assign({ chat_id: $json.chatId,"
                     " text: 'DBeats: ' + $json.antwort, parse_mode: 'HTML', disable_web_page_preview: true },"
    ...
    n("Ausfuehren", "@n8n/n8n-nodes-langchain.agent", 2.2, [160, -140], {
        ...
        "options": {"systemMessage": SYSTEM_AUSFUEHREN, "maxIterations": 4},
    ...
    n("Nacharbeiten", "@n8n/n8n-nodes-langchain.agent", 2.2, [2100, -60], {
        ...
        "options": {"systemMessage": SYSTEM_AUSFUEHREN, "maxIterations": 3},
        "hasOutputParser": False,
    }, retryOnFail=True, maxTries=2, waitBetweenTries=3000, onError="continueRegularOutput",
    ...
    n("Senden", "n8n-nodes-base.httpRequest", 4.2, [3000, 100], {
        ...
        "sendHeaders": True, "headerParameters": {"parameters": JSON_KOPF},
        "sendBody": True, "specifyBody": "json",
        "jsonBody": ("={{ JSON.stringify(Object.assign({ chat_id: $json.chatId, text: 'DBeats: ' + $json.antwort,"
    ...
    n("Senden (Kurzmeldung)", "n8n-nodes-base.httpRequest", 4.2, [600, -520], {
        ...
        "sendHeaders": True, "headerParameters": {"parameters": JSON_KOPF},
        "sendBody": True, "specifyBody": "json",
        "jsonBody": ("={{ JSON.stringify({ chat_id: $json.chatId, text: 'DBeats: ' + $json.antwort,"
                     " parse_mode: 'HTML', disable_web_page_preview: true }) }}"),
        "options": {"timeout": 20000},
    }, onError="continueRegularOutput",
...
LEGENDE_BOT = """## Radio - Telegram-Agent (Deadline Beats) - zweisprachig (DE/EN)
...
Erzeugt von werkzeuge/agent-wf-bauen.py - nie von Hand aendern.
Aendern: agent-patchen.sh --inhalt ... dann agent-einspielen-nur.sh.
Doku: README.md Abschnitt 8 bis 12 und werkzeuge/README.md."""
...
Knopf oder Text -> **Dienst Art** -> **Meldung?** -> Modul im Dienst radio-tts.
...
W_KONFIG = "Konfiguration"
W_KONFIG_NAME = "Konfiguration - alle Werte"
...
KONFIG = {
    ...
    # Charakter der Stimme aus ../charakter.md: faerbt die GESPROCHENEN Ansagen
    # (der Planer formuliert damit freie Ansagen). Leer = keine eigene Rolle.
...
//  die Aufgabentexte des Sprachmodells und der Charakter der Stimme fuer
//  gesprochene Ansagen (Feld "charakter", aus ../charakter.md). Die Ablaufe holen
...
def konfig_platzhalter(werte: dict) -> dict:
    ...
    v["dienst"]["adresse"] = "http://DEIN-DIENST:8881"
...
dokunotiz(konfiguration, W_KONFIG_NAME, [
    ...
    "Aendern: werkzeuge/agent-wf-bauen.py (KONFIG) -> agent-einspielen-nur.sh",
])
...
dokunotiz(agent, "Radio - Telegram-Agent", [
    ...
    "Der Plan ist die Quelle der Anordnung: werkzeuge/agent-wf-bauen.py (ANORDNUNG, BEREICHE, KURZNOTIZ).",
...
dokunotiz(werkzeuge[0], "Werkzeug - Radio", [
    ...
    "Der Plan ist die Quelle: werkzeuge/agent-wf-bauen.py (W_ANORDNUNG, W_BEREICHE).",
    "Aendern/Pruefen wie beim Agenten; Beschreibung: HANDBUCH.md, HANDBUCH.md, ANHANG/n8n-oberflaeche.html",
])
dokunotiz(werkzeuge[1], "Werkzeug - AzuraCast", [
    ...
    "Der Plan ist die Quelle: werkzeuge/agent-wf-bauen.py (W_ANORDNUNG_AZ, W_AZ_BEREICHE).",
    "Aendern/Pruefen wie beim Agenten; Beschreibung: HANDBUCH.md, ANHANG/n8n-oberflaeche.html",
])
dokunotiz(werkzeuge[2], "Werkzeug - Meldungen", [
    "Unterschnittstelle fuer Ansage und Postfach: Recherche, freie Ansage, Meldung sprechen/verwerfen.",
    "Spricht ueber den Dienst radio-tts (Piper + DJ-Hafen), legt Meldungen im Postfach ab.",
    "Der Plan ist die Quelle: werkzeuge/agent-wf-bauen.py (W_MELD_ANORDNUNG, W_MELD_BEREICHE).",
