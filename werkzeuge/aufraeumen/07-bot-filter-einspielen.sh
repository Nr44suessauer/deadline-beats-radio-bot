#!/bin/bash
# Spielt den gepatchten Such-Ablauf in n8n ein (ohne Neubau der Ablaeufe,
# damit keine Zugangsdaten gebraucht werden).
# Aufruf:  bash 07-bot-filter-einspielen.sh <datei.json> [Ablauf-Kennung]
set -euo pipefail

CFG=~/.ssh/config
DATEI=${1:-<dokuordner>/werkzeuge/aufraeumen/wf-radio-neu.json}
ABLAUF=${2:-RadioWerkzeug}
